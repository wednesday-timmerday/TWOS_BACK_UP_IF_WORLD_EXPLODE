...
class BoxEngine:  # ! still mid name but whatever
    ...
    def __init__(self, world_loader, preset="default_blue"):

        self.world_loader = world_loader

        self.part_loader = Loader(f"ui/boxEngine/parts/{preset}/")
        self.btn_loader = Loader("ui/boxEngine/parts/")
        ...
        self.corner_topleft = pygame.transform.flip(self.corner_left, False, True)
        self.corner_topright = pygame.transform.flip(self.corner_left, True, True)
    ...
    def refresh(self):
        ...
        for btn_id in range(3):
            ...
            path = self.btn_loader.load(f"btn_{btn_id + 1}.png")
    ...
    def create_box(self, box_coords):
        ...
        self.corner_topleft_pos = (x, y)
        self.corner_topright_pos = (x+ h, y)
        ...
        self.filler_under_scaled = pygame.transform.scale(
            ...
            (w - self.corner_left.get_width(), h + self.corner_left.get_height())

        )

        self.filler_under_pos = (x + self.corner_left.get_width(), y)

        filler_scaled_h = w-self.corner_left.get_height()

        self.filler_scaled = pygame.transform.scale(

            self.filler,

            (w + self.corner_left.get_width(), filler_scaled_h)

        )

        self.filler_pos = (x, y + self.corner_left.get_height())
    ...
    def draw(self, screen, with_btns=False):
        ...
        screen.blit(self.corner_topleft, (self.corner_topleft_pos[0], self.corner_topleft_pos[1]))
        screen.blit(self.corner_topright, (self.corner_topright_pos[0], self.corner_topright_pos[1]))
