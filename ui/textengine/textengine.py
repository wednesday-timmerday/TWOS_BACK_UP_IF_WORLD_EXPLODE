...
class TextEngine:
    ...
    def draw(self, x, y, text_color=(0,0,0), outline_color=(0,0,0),
        ...
        for line in lines:
            ...
            for ch in line:
                # Reset color/shake for this character
                self.current_color = text_color
                self.shake_intensity = 0

                # Apply per-character effects
                for eff in self.char_effects.get(idx, []):
                ...
                wave = 0
                if "roar" in self.char_effects.get(idx, []):
                    wave = int(math.sin(self.time * 4 + word_index * 0.6) * 6)

                # Render character with outline
                char_color = self.current_color  # uses effect if any, else default inside
                char_surf = self.font.render(ch, True, char_color)

                # Draw outline by rendering the char multiple times around
                for dx in range(-outline_width, outline_width+1):
                    for dy in range(-outline_width, outline_width+1):
                        ...
                        surf.blit(
                            outline_surf,
                            (
                                offset_x + dx + random.randint(-self.shake_intensity, self.shake_intensity),
                                offset_y + dy + wave + random.randint(-self.shake_intensity, self.shake_intensity)
                            )
                        )
                ...
                surf.blit(
                    ...
                    (
                        ...
                        offset_y + wave + random.randint(-self.shake_intensity, self.shake_intensity)
                    )
                )
            ...
            idx += 1  # newline spacing

        # -----------------------------
        # Draw choices below text
        # -----------------------------
