...
def ensure_options_file():
    loader = Loader("ui/menu")
    options_path = loader.load(OPTIONS_FILE)

    if not options_path or not os.path.exists(os.path.dirname(options_path)):
        os.makedirs(os.path.join(os.getcwd(), "assets", "ui", "menu"), exist_ok=True)
        options_path = os.path.join(os.getcwd(), "assets", "ui", "menu", OPTIONS_FILE)

    default_options = {"fullscreen": False, "master_volume": 0.5}
...
class Menu:
    ...
    def __init__(self, settings=None):
        ...
        self.settings_options = ["Fullscreen", "Master Volume", "Controllers", "Go back", "Reset Game"]
        ...
        self.joystick = None
        self.used = False
        self._init_first_joystick_if_present()
    ...
    def _init_first_joystick_if_present(self):
        ...
        while self.running:
            ...
            for i, rect in enumerate(self.settings_rects):
                if rect.collidepoint(mouse_pos):
                    ...
                    print("Selected setting via mouse:", self.selected_setting)
            ...
            for event in pygame.event.get():
                ...
                if event.type in (pygame.JOYDEVICEADDED, pygame.JOYDEVICEREMOVED):
                    try:
                        ...
                        print(f"[Menu] Joystick init error: {e}")
    ...
    def is_sun_down(self):
        ...
        if self.cached_sun is None:
            # if no location, don't block — return False (show sun)
            with self.location_lock:
    ...
    def draw_settings(self, screen):
        ...
        for i, option in enumerate(self.settings_options):
            ...
            if option == "Master Volume":
                ...
                self.slider_rect = pygame.Rect(slider_x, slider_y, self.slider_width, self.slider_height)

                # draw bar
                pygame.draw.rect(screen, (80, 80, 80), self.slider_rect, border_radius=6)
                # draw filled portion
                ...
                self.knob_pos = (int(knob_x), int(knob_y))

                # draw knob (circle)
                pygame.draw.circle(screen, (230, 230, 230), self.knob_pos, self.knob_radius)
                # knob outline
                pygame.draw.circle(screen, (120, 120, 120), self.knob_pos, self.knob_radius, 2)
    ...
    def _handle_settings_mouse_down(self, pos):
        ...
        # First, check if slider exists and mouse is on knob or bar
        if self.slider_rect:
            knob_x, knob_y = self.knob_pos if self.knob_pos else (0, 0)
            # distance to knob
    ...
    def _handle_settings_mouse_motion(self, pos):
        """If dragging the slider, update volume in real-time."""
        if self.dragging_slider:
            mx, my = pos
            self._set_volume_by_mouse(mx)

    def _set_volume_by_mouse(self, mx):
        """Set volume based on absolute mouse x position over the slider bar."""
        if not self.slider_rect:
            return
        rel_x = mx - self.slider_rect.left
        rel_x = max(0, min(rel_x, self.slider_rect.width))
        new_vol = rel_x / float(self.slider_rect.width)
        # clamp and round a bit for neatness
        new_vol = round(max(0.0, min(new_vol, 1.0)), 3)
        if abs(new_vol - self.settings.get("master_volume", 0.0)) > 0.0001:
            self.settings["master_volume"] = new_vol
            pygame.mixer.music.set_volume(new_vol)
            # update knob_pos for immediate visuals
            fill_w = int(self.slider_width * new_vol)
            knob_x = self.slider_left + fill_w
            knob_y = self.slider_top + self.slider_height // 2
            self.knob_pos = (int(knob_x), int(knob_y))
