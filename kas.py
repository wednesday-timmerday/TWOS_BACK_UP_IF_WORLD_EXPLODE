"""
=============================================================================
 Realtime Desktop Audio Analyzer — BPM & Nootwaarde Detectie
=============================================================================
 Vereisten:
...
# EMA smoothing — hoger = sneller reageren op aanvallen
SMOOTH_ALPHA      = 0.40
...
# Kalibratie: hoeveel peaks voor de initiële BPM
...
class AudioAnalyzer:
    ...
    # Nootwaarden in beats (relatief t.o.v. één kwartnoot = 1 beat)
    NOTE_TABLE = {
        "32e noot":     0.125,
        "16e noot":     0.25,
        "achtste noot": 0.5,
    ...
    def __init__(self):
        ...
        # Beat-grid state
        self.beat_duration_s  = None   # duur van één kwartnoot in seconden
        self.bpm              = None
    ...
    def _update_bpm(self, measured_interval_s: float):
        """
        Pas BPM voorzichtig aan — maximaal BPM_MAX_DRIFT per stap.
    ...
    def process_block(self, indata: np.ndarray, frames: int, time_info, status):
        ...
        # ── Analysefase: meet interval t.o.v. vorige geaccepteerde beat ────────
        ...
        if 0.8 < beats_measured < 1.2:
            # Dit is een kwartnoot — gebruik hem voor BPM-update
            self._update_bpm(interval)
        elif 1.8 < beats_measured < 2.2:
            # Halve noot — deel door 2 voor beat-duur
...
def main():
    ...
    print(f"[CONFIG] BPM_MAX_DRIFT   = ±{BPM_MAX_DRIFT*100:.0f}% per beat\n")
    ...
    if analyzer._max_raw_seen > 0:
        ...
        if analyzer._max_raw_seen < THRESHOLD:
            print("          !! Signaal haalde threshold nooit — verlaag THRESHOLD !!")
