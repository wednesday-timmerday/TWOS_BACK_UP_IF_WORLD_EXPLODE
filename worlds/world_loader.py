...
def get_enemy_class(enemy_type_name, *args, **kwargs):
    ...
    if enemy_type_name in _enemy_class_cache:
        ...
        try:
            ...
            print(
                f"[get_enemy_class] Failed to instantiate cached class '{enemy_type_name}': {e}"
            )
    ...
    try:
        ...
        sprites_path = os.path.join(
            os.path.dirname(__file__), "..", "sprites", enemy_type_name
        )
        ...
        spec = importlib.util.spec_from_file_location(
            f"sprites.{enemy_type_name}", module_path
        )
...
class World_loader:
    ...
    def load_layers(self):
            ...
            if img is None:
                ...
                if chunk_paths:
                    ...
                    info = {
                        ...
                        "base_name": base_name,
                        "scaled_chunks": [None]
                        * len(chunk_paths),  # will hold surfaces once created
                    }
            ...
            if img is not None and img.get_height() != self.Screen_resolution[1]:
                try:
                    ...
                    img = pygame.transform.scale(
                        img, (new_w, self.Screen_resolution[1])
                    )
        ...
        print(
            f"Loaded {len(self.layer_info)} layer metadata entries for level {self.current_level}"
        )
    ...
    def _render_collision_background(self):
        ...
        self.collision_background = pygame.Surface(
            (total_width, height), pygame.SRCALPHA
        )
    ...
    def _get_chunk_surface(self, layer_idx, chunk_idx):
        ...
        try:
            ...
            sf = info.get(
                "scale_factor", target_h / img.get_height() if img.get_height() else 1
            )
            ...
            print(
                f"[get_chunk] failed to load/scale chunk {layer_idx}:{chunk_idx}: {e}"
            )
    ...
    def load_enemies(self):
        ...
        for platform_data in level_platform:
            ...
            self.platforms.append(
                {"surf": platform_surf, "x": platformx, "y": platformy}
            )
        ...
        for enemy_data in level_enemies:
            ...
            if enemy_type.startswith("physics_obj_"):
                ...
                try:
                    ...
                    print(
                        f"[load_enemies] Failed to spawn physics object '{phys_name}': {e}"
                    )
            ...
            # -------------------------
                ...
                try:
                    ...
                    if (
                        enemy_type == "lantern"
                        or enemy_type == "hammer"
                        or enemy_type == "shadowrock"
                    ):
    ...
    def build_collision_mask_new(self):
                ...
                try:
                    ...
                    if surf.get_height() != self.Screen_resolution[1]:
                        ...
                        surf = pygame.transform.scale(
                            surf, (new_w, self.Screen_resolution[1])
                        )
                    ...
                    print(
                        f"[build_collision_mask_new] failed to load deferred layer {p}: {e}"
                    )
        ...
        total_w = (
            ...
            if first_info["type"] == "chunks"
            else first_info["surface"].get_width()
        )
        ...
        for idx, info in enumerate(self.layer_info):
            ...
            coll_layer_names |= {
                str(i) for i in self.level_data.get("coll_layers", [3, 4])
            }
                ...
                if info["type"] == "full":
                    ...
                    if surf:
                        try:
                            ...
                            print(
                                f"[build_collision_mask_new] failed scaling full surface: {e}"
                            )
                    ...
                    for ci, chunk_path in enumerate(info["chunks"]):
                        ...
                        try:
                            ...
                            if downsample > 1:
                                ...
                                scaled_w = max(
                                    1, int(info["chunk_widths"][ci] // downsample)
                                )

                                small_chunk = pygame.transform.smoothscale(
                                    ch, (scaled_w, small_h)
                                )
                            ...
                            print(
                                f"[build_collision_mask_new] chunk {idx}:{ci} failed: {e}"
                            )
    ...
    def draw_black_layer(self, screen, player_or_x, player_y=None):
        ...
        # --------- PLAYER LIGHT ---------
        # Uses _player_light_radius so it can be changed at runtime.
        ...
        if -cull < cx < screen_w + cull and -cull < cy < screen_h + cull:
            ...
            self._light_overlay.blit(
                player_mask, rect, special_flags=pygame.BLEND_RGBA_SUB
            )

        # --------- OTHER LIGHT SOURCES ---------
        # Each source uses its own radius and alpha, fetched from the cache.
        ...
        for light in self.light_sources:
            ...
            # Cull lights outside the viewport (use radius as margin)
            ...
            self._light_overlay.blit(
                mask, rect, special_flags=pygame.BLEND_RGBA_SUB
            )
    ...
    def draw_world(self, screen, player_x, player_y):
        ...
        if self.static_bg_far:
            ...
            clip_rect = pygame.Rect(
                int(visible_x), 0, self.Screen_resolution[0], self.Screen_resolution[1]
            )
        ...
        if self.static_bg_near:
            screen.blit(
                self.static_bg_near, (-cam_x, 0)
            )  # float offset — pygame truncates at blit
    ...
    def draw_timer(self, screen):
        shit_to_render = self.timer_font.render(
            f"Timer: {round(self.time_to_timer - self.Time_left_in_timer_that_times_the_time_in_a_timely_manner)}",
            True,
            (255, 255, 255),
        )
        if (
            round(
                self.time_to_timer
                - self.Time_left_in_timer_that_times_the_time_in_a_timely_manner
            )
            <= 0
            and self.is_timer_active
            and self.level_data.get("timer", None) != None
        ):
            self.player.die(self)
            print(
                f"1: {self.time_to_timer}, 2: {self.Time_left_in_timer_that_times_the_time_in_a_timely_manner}"
            )
        if (
            self.level_data.get("timer", None) != None
    ...
    def change_level(self, level_id, player):
        ...
        # Clear mask cache too — radii may differ between levels and memory
        # should not grow unboundedly across many level transitions.
        self._light_mask_cache = {}
        self._big_light_mask_dark = self._get_light_mask(self._player_light_radius)
