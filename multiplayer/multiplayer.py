...
# ---------------------------------------------------------------------------
_sprite_cache: dict = {}   # anim_name → [Surface, ...]
...
class MP:
    def __init__(self, player, room_id: str = "default", name: str = "???", join_mode: bool = False):
        ...
        # remote players: uid → {x, y, anim, dir, curr_frame, name, _last_seen}
    ...
    def draw(self, renderer: pygame.Surface, world_loader):
        ...
        for p in players_snapshot:
            ...
            # blit so sprite's bottom-centre lands on (sx, sy) — same as midbottom
    ...
    async def _network_task(self):
        ...
        while self._running:
            try:
                ...
                print(f"[MP] Connection error: {exc}  — retrying in 3s")
                with self._lock:
    ...
    async def _receiver(self, ws):
        ...
        # socket closed → raise so _network_task reconnects
