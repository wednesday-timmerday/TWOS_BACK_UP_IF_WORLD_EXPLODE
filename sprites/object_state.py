        ...
        for obj_id, data in save_data.items():
            if obj_id in self.objects:
                try:
                    ...
                    print(f"[ObjectStateManager] ✓ Loaded state for object {obj_id}")
                except Exception as e:
                    print(f"[ObjectStateManager] ✗ Failed to load state for object {obj_id}: {e}")
            else:
                print(f"[ObjectStateManager] ✗ Object {obj_id} not found in registry (have {list(self.objects.keys())})")
