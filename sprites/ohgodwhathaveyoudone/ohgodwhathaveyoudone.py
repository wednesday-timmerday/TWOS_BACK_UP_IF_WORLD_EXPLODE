...
class Ohboy(StateSerializable, EnemyBase):
    def __init__(self):
        StateSerializable.__init__(self)
        EnemyBase.__init__(self, "ohgodwhathaveyoudone", frame_count=1, scale_percentage=(10, 10))
        self.object_type = "ohgodwhathaveyoudone"
        self.world_x = 400  # Initial position
        self.world_y = 300
        self.pos = [self.world_x, self.world_y]
    
    def serialize_state(self):
        """Save Ohboy state including position and frame"""
        return {
            "x": int(self.world_x),
            "y": int(self.world_y),
    ...
    def deserialize_state(self, state):
        """Restore Ohboy state including position and frame"""
