...
class ShadowRock(StateSerializable, EnemyBase):
    def __init__(self, world_loader=None):
        ...
        EnemyBase.__init__(
            self, "ShadowRock", frame_count=2, scale_percentage=(100, 100)
        )
    ...
    def draw_in_world(self, surface, cam_x, cam_y):
        if self.world_loader.current_level == 3 and not self.added:
            print(self.frames[self.current_frame].get_height())
            self.world_loader.add_light_source(self, 100, offset=(0, -self.frames[self.current_frame].get_height() // 2))
            self.added = True
        ...
        try:
            self.blit_frame_from_atlas(
                surface,
                self.current_frame,
                (
                    screen_x - self.frames[self.current_frame].get_width() // 2,
                    screen_y - self.frames[self.current_frame].get_height(),
                ),
            )
    ...
    def draw(self, surface, x, y):
        ...
        try:
            self.blit_frame_from_atlas(
                surface,
                self.current_frame,
                (x - frame.get_width() // 2, y - frame.get_height()),
            )
