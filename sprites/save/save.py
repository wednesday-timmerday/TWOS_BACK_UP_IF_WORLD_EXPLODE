...
class SaveOBJ(StateSerializable, EnemyBase):
    ...
    def save_game(self, world, player, triggered_once):
        ...
        try:
            ...
            if state_manager:
                ...
                print(f"[SaveOBJ.save_game] âœ“ Saved state for {len(level_states)} objects in {level_key}")
            ...
            print(f"[SaveOBJ.save_game] âœ— Failed to save object states: {e}")
    ...
    def apply_object_states(self, world):
        ...
        if not hasattr(self, "_full_save") or not self._full_save:
            print(f"[SaveOBJ.apply_object_states] âœ— No save data loaded")
            return
        
        # Get object states for the current level
        object_states = self._full_save.get("object_states", {})
        if not object_states:
            print(f"[SaveOBJ.apply_object_states] âœ— No object_states in save file")
            return
        ...
        if not level_object_states:
            print(f"[SaveOBJ.apply_object_states] âœ— No states for {level_key}")
            return
        
        state_manager = getattr(world, "object_state_manager", None)
        if not state_manager:
            print(f"[SaveOBJ.apply_object_states] âœ— No state_manager in world")
            return
        
        try:
            print(f"[SaveOBJ.apply_object_states] â†’ Applying {len(level_object_states)} saved states to {level_key}")
            state_manager.load_all_states(level_object_states)
        except Exception as e:
            print(f"[SaveOBJ.apply_object_states] âœ— Failed to apply object states: {e}")
